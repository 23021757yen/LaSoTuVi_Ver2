...
const API_BASE_URL = process.env.NEXT_PUBLIC_TUVI_API_BASE_URL ?? "http://localhost:8000";

function FieldLabel({ children }: { children: React.ReactNode }) {
  return <div className="mb-1.5 text-sm font-semibold text-[#000000]">{children}</div>;
}
...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = React.useState<"lap" | "huongdan">("lap");
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = (await response.json()) as TuViResponse | { detail?: string; error?: string };

      if (!response.ok) {
        const message = ("detail" in payload && typeof payload.detail === "string" && payload.detail)
      ...
      const message = error instanceof Error ? error.message : "Lỗi kết nối tới backend API.";
      setApiError(message);
      setLaSoData(null);
    } finally {
      setLoading(false);
  ...
  <div className="mx-auto max-w-7xl px-4 grid gap-10 lg:grid-cols-[2fr_1fr]">
    ...
    <div>
      ...
      <div className="text-2xl md:text-3xl text-[#000000]">
        LÁ SỐ TỬ VI
      </div>

      <div className="relative border-b border-[#E0E0E0] mb-6">
        <div className="flex">

          <button
            onClick={() => setActiveTab("lap")}
            className={`px-6 py-2 font-semibold ${
              activeTab === "lap" ? "text-[#DA251D]" : "text-gray-500"
            }`}
          >
            Lập lá số tử vi
          </button>
          ...
          <button
            ...
            className={`px-6 py-2 font-semibold ${
              activeTab === "huongdan" ? "text-[#DA251D]" : "text-gray-500"
            }`}
          >
            Xem hướng dẫn
          </button>
        ...
        {/* line animation */}
        <div
          className="absolute bottom-0 h-0.75 bg-[#DA251D] transition-all duration-300"
          style={{
            width: "130px",
            transform:
              activeTab === "lap"
      ...
      <div className="mx-auto mt-10 max-w-4xl border-8 border-double border-[#DA251D] p-2 mb-12 bg-[#FEF1D9]">

      {activeTab === "lap" && (
        <section id="xem-la-so" className="px-6 md:px-10 py-10">
          <div className="text-center">
            <h2 className="text-[24px] font-extrabold leading-none text-[#DA251D] md:text-[32px]">Lập Lá Số Tử Vi</h2>
          </div>

          <form onSubmit={onSubmit} className="mt-10 space-y-1">
            <div className="grid gap-4 md:grid-cols-[1fr_220px]">
              <div>
                <FieldLabel>Họ và tên <span className="text-red-500">*</span></FieldLabel>
                <Input
                  value={name}
              ...
              <div>
                <FieldLabel>Giới tính <span className="text-red-500">*</span></FieldLabel>
                <div className="mt-0.5 flex gap-2 ">
                  <OptionPill name="gender" value="nu" checked={gender === "nu"} onChange={setGender} label="Nữ" />
                  <OptionPill name="gender" value="nam" checked={gender === "nam"} onChange={setGender} label="Nam" />
                </div>
              </div>
            </div>

            <div>
              <FieldLabel>Loại tin <span className="text-red-500 ">*</span></FieldLabel>
              <Select value={calendarType} onChange={(e) => setCalendarType(e.target.value as CalendarType)} required>
  ...
  <div>
    <FieldLabel>Năm <span className="text-red-500">*</span></FieldLabel>
  ...
  <div>
    <FieldLabel>Tháng <span className="text-red-500">*</span></FieldLabel>
  ...
  <div>
    <FieldLabel>Ngày <span className="text-red-500">*</span></FieldLabel>
  ...
  <div className="grid grid-cols-2 gap-3">
    <div>
      <FieldLabel>Giờ <span className="text-red-500">*</span></FieldLabel>
    ...
    <div>
      <FieldLabel>Phút <span className="text-red-500">*</span></FieldLabel>
  ...
  <div>
    <FieldLabel>Xem năm hạn <span className="text-red-500">*</span></FieldLabel>
    <Select value={viewYear} onChange={(e) => setViewYear(Number(e.target.value))}>
      {years.map((y) => (
        <option key={y} value={y}>{y}</option>
            ...
            <div>
              ...
              <div className="mt-0.5 flex flex-wrap gap-2">
                <OptionPill name="dst" value="co" checked={dst === "co"} onChange={setDst} label="Có cảnh báo" />
                <OptionPill name="dst" value="khong" checked={dst === "khong"} onChange={setDst} label="Không cảnh báo" />
              </div>
            </div>
            ...
            <div className="mx-auto mt-7 max-w-4xl">
              <div className="mb-3 flex items-center justify-end">
                <button
                  ...
                  className={cn(
                    "inline-flex h-9 items-center justify-center rounded-md bg-[#0B5F46] px-4 text-sm font-semibold text-white",
                    "disabled:cursor-not-allowed disabled:opacity-70",
                  )}
                >
              ...
              <div ref={chartExportRef} className="overflow-hidden rounded border border-zinc-300 bg-white">
                <div className="h-9 rounded-sm bg-linear-to-r from-[#0B5F46] to-[#E4E9EA] px-4 text-sm font-semibold leading-9 text-white">
        ...
        {activeTab === "huongdan" && (
        <section id="huong-dan" className="mx-auto max-w-6xl px-4 py-12 md:py-16">
          <div className="text-center">
            <h2 className="text-[24px] font-extrabold leading-none text-[#DA251D] md:text-[32px]">Xem Hướng Dẫn</h2>
          </div>
          <div className="relative mt-14">
            <div className="pointer-events-none absolute left-[12%] right-[12%] top-7 hidden h-0.5 bg-[#DA251D] md:block" />
            <div className="grid gap-10 md:grid-cols-4">
              {steps.map((step, idx) => (
                <div key={step.title} className="relative text-center">
                  <div className="relative mx-auto h-16 w-16 rounded-full bg-linear-to-r from-[#DA251D] to-[#EFA3A0]">
                    <span className="absolute -right-1 -top-1 inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#F4C06A] text-[11px] font-bold text-[#DA251D]">
                      {idx + 1}
                    </span>
                  ...
                  <h3 className="mt-4 text-[16px] font-bold text-[#DA251D] md:text-[18px]">{step.title}</h3>
                  <p className="mx-auto mt-2 max-w-60 text-[14px] leading-5 text-black md:text-[14px] md:leading-6">
                    {step.desc}
                  </p>
                </div>
              ))}
    ...
    <div className="my-15">
      ...
      <div className="border-8 border-double border-[#DA251D] p-2 bg-[#FEF1D9] my-1">
        <div className="text-[24px] font-extrabold leading-none text-[#DA251D] md:text-[32px] text-center">
          Kiến thức tử vi
        </div>
    ...
    <div className="bg-[#ffffff] rounded-xl p-5 shadow-sm hover:shadow-md transition">
      ...
      <p className="mt-4 text-black leading-relaxed text-[14px]">
        Sao Thiên Tướng: là một trong ngũ đại chính tinh của khoa tử vi,
        thuộc chòm sao Bắc Đẩu Tinh với hành Thủy, mang tính chất quyền tinh.
      </p>
    </div>
    ...
    <div className="bg-[#ffffff] rounded-xl p-5 shadow-sm hover:shadow-md transition">
      ...
      <p className="mt-4 text-black leading-relaxed text-[14px]">
        Sao Tử Vi: là sao đứng đầu trong hệ thống chính tinh của khoa Tử Vi,
  được xem như "đế tinh", tượng trưng cho quyền lực, uy nghi và khả năng
  lãnh đạo. 
      </p>
    </div>
    ...
    <div className="bg-[#ffffff] rounded-xl p-5 shadow-sm hover:shadow-md transition">
      ...
      <p className="mt-4 text-text-[14px] leading-relaxed text-[14px]">
         Sao Thiên Phủ: là một chính tinh thuộc hành Thổ trong khoa Tử Vi,
  mang tính chất kho tàng và tài phúc. Sao này tượng trưng cho sự ổn định,
  phúc hậu, khả năng quản lý tài sản và tích lũy của cải.
      </p>
    </div>
  </div>
